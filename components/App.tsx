...
export default function App(){
 const [data,setData]=useState<ParseResult>(sample); const [busy,setBusy]=useState(false); const [fileName,setFileName]=useState(""); const sheetRef=useRef<HTMLDivElement>(null);
 ...
 async function pptx(){const PptxGenJS=(await import("pptxgenjs")).default; const ppt=new PptxGenJS();ppt.layout="LAYOUT_A4";const s=ppt.addSlide();s.background={color:"FFFFFF"};s.addText(`MAXI ZOO CHEAT SHEET – ${data.period}`,{x:.3,y:.2,w:7.7,h:.4,bold:true,fontSize:20,color:"00542B"});let y=.8; for(const [c,ps] of grouped){s.addText(c,{x:.3,y,w:7.7,h:.3,bold:true,fontSize:14,color:"FFFFFF",fill:{color:colors[c].slice(1)}});y+=.35;for(const p of ps){s.addText(`${p.brand} ${p.product} — ${p.offer}${p.price?` (${p.price})`:""}`,{x:.45,y,w:7.3,h:.22,fontSize:9,breakLine:false});y+=.23;}y+=.08;}await ppt.writeFile({fileName:"maxi-zoo-cheat-sheet.pptx"});}
 return <main>
  <header><div><div className="eyebrow">MAXI ZOO</div><h1>Cheat Sheet Generator</h1><p>Upload een folder, controleer de herkende promoties en exporteer onmiddellijk.</p></div><div className="status"><Sparkles size={18}/> Geavanceerde Maxi Zoo-modus</div></header>
  <section className="toolbar">
   ...
   <button onClick={png}><Download size={17}/>PNG</button><button onClick={pdf}><Download size={17}/>PDF</button><button onClick={excel}><FileSpreadsheet size={17}/>Excel</button><button onClick={pptx}><Presentation size={17}/>PowerPoint</button>
  </section>
  ...
  <div className="workspace">
   ...
   <section className="preview"><div className="sheet" ref={sheetRef}>
    ...
    <div className="grid">{grouped.map(([c,ps])=><section className="cat" key={c} style={{"--cat":colors[c]} as any}><h3>{c}</h3>{ps.map(p=><div className="row" key={p.id}><div><b>{p.brand}</b>{p.brand&&" "}{p.product}{p.notes&&<small>{p.notes}</small>}</div><span>{p.offer}{p.price&&<em>{p.price}</em>}</span></div>)}</section>)}</div>
